...
local function parsePluralExpression(expr)
    ...
    if expr:find("n%%100") or expr:find("nplurals=3") then
        return function(n)
            -- Честный и чистый расчет множественного числа на Lua
            if n % 10 == 1 and n % 100 ~= 11 then
                return 0 -- первая форма (1 день)
            elseif n % 10 >= 2 and n % 10 <= 4 and (n % 100 < 10 or n % 100 >= 20) then
                return 1 -- вторая форма (2, 3 дня)
            else
                return 2 -- третья форма (0, 5, 11 дней)
            end
        end
    end
    ...
    local function translateTernary(s)
        -- Очищаем внешние скобки, чтобы findQuestion не "слепла"
        s = s:gsub("^%s*%((.*)%)%s*$", "%1")
        ...
        -- Ищем двоеточие строго на текущем уровне вложенности
        local depth = 0
        local colon = nil
        for i = q + 1, #s do
            local c = s:sub(i, i)
            ...
            elseif c == ")" then
                depth = depth - 1
            elseif c == "?" and depth == 0 then
                -- Новый тернарник на том же уровне — увеличиваем глубину для двоеточия
                depth = depth + 1
            elseif c == ":" and depth == 0 then
                colon = i
                break
        ...
        -- КРИТИЧЕСКИЙ МОМЕНТ: оборачиваем translateTernary(falsy) в строгие скобки,
        -- чтобы приоритет "and / or" в Lua не ломал вложенные условия!
        return "((" .. translateTernary(cond) .. ") and (" .. translateTernary(truthy) .. ") or (" .. translateTernary(falsy) .. "))"
    end

    -- Стандартизируем операторы перед парсингом
    expr = expr:gsub("!=",   "~=")
    expr = expr:gsub("&&",   " and ")
    expr = expr:gsub("%|%|", " or ")
